...
struct workqueue_struct *new_suspend_work_queue;
...
static int __init power_suspend_init(void)
{
	...
	new_suspend_work_queue = create_singlethread_workqueue("p-suspend");

	if (new_suspend_work_queue == NULL) {
		return -ENOMEM;
	}
...
static void __exit power_suspend_exit(void)
{
	...
	destroy_workqueue(new_suspend_work_queue);
} 
